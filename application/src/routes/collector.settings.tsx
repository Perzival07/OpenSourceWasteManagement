// src/routes/collector.settings.tsx
import { createFileRoute, useNavigate } from "@tanstack/react-router";
import { useState, useEffect } from "react";
import { User, Mail, Phone, Save, Key, MapPin, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/collector/settings")({
  component: CollectorSettings,
});
...
function CollectorSettings() {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    const rules = {
      ...
      zone: validators.required, // Collectors must have a zone
    };
    const errs = validateFields(rules, { display_name: displayName, phone_number: phoneNumber, zone });
    setFieldErrors(errs);
    ...
    try {
      const updated = await apiFetch("/users/me", {
        method: "PUT",
        body: { display_name: displayName, phone_number: phoneNumber, zone },
      });
      await refreshMe();
  ...
  return (
    <>
      <PageHeader title="Account Settings" subtitle="Manage your profile and work zone" />
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Update your personal details and assigned zone.
          </p>
          ...
          <form onSubmit={handleProfileUpdate} className="space-y-4">
            ...
            <FormField
              ...
              hint="Your collection zone – required for task assignments."
            />
